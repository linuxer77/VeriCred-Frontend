...
export default function UniversityDashboard() {
  ...
  const [studentsDebug, setStudentsDebug] = useState<string | null>(null);
      ...
      try {
        ...
        if (storedUser && token) {
          ...
          if (response.ok) {
            ...
            // Prefer authoritative students list from /students endpoint if available
            try {
              // Try relative endpoint first (works when frontend and backend are proxied)
              const attemptFetch = async (url: string) => {
                try {
                  ...
                  console.debug(`Fetch to ${url} failed:`, err);
                  return null;
                }
              };

              let studentsResp = await attemptFetch("/students");
              if (!studentsResp || !studentsResp.ok) {
                console.debug(
                  "/students (relative) failed, trying http://localhost:8080/students"
                );
                studentsResp = await attemptFetch(
              ...
              if (studentsResp && studentsResp.ok) {
                ...
                console.debug("Fetched students:", studentsData);
                setStudentsDebug(JSON.stringify(studentsData, null, 2));

                const studentsArray = Array.isArray(studentsData)
                  ? studentsData
                  : [studentsData];

                const mappedStudents: Student[] = studentsArray.map(
                  (s: any) => ({
                    id: String(
                      s.id ?? s.student_id ?? s.email ?? s.metamask_address
                    ),
                    name:
                      `${s.first_name ?? ""} ${s.last_name ?? ""}`.trim() ||
                      s.email ||
                      "Unknown",
                    universityId: s.student_id ?? String(s.id ?? ""),
                    walletAddress:
                      s.metamask_address ?? s.Account?.metamask_address ?? "",
                    eligibilityStatus: s.is_verified
                      ? ("eligible" as const)
                      : ("pending_review" as const),
                    mintingStatus: "none" as const,
                    joinDate: s.Account?.created_at ?? new Date().toISOString(),
                    lastActivity:
                      s.Account?.last_login_at ?? new Date().toISOString(),
                  })
                );

                setStudents(mappedStudents);

                // Update stats based on fetched students and any credentials included in /university response
                setAnimatedStats({
                  students: mappedStudents.length,
                  credentials:
                    data.credentials?.reduce(
                      (sum: number, cred: any) => sum + (cred.totalIssued || 0),
                      0
                    ) || 0,
                  pending:
                    mappedStudents.filter((s) => s.mintingStatus === "pending")
                      .length || 0,
                });
              } else {
                console.debug(
                  "students endpoint not available, falling back to university response students"
                );
                setStudents(data.students || []);
                setStudentsDebug(JSON.stringify(data.students || [], null, 2));
                setAnimatedStats({
                  ...
                  pending:
                    data.students?.filter(
                      (s: any) => s.mintingStatus === "pending"
                    ).length || 0,
                });
              }
            } catch (err) {
              console.error("Failed to fetch students endpoint:", err);
              // Fallback
              setStudents(data.students || []);
              setStudentsDebug(String(err));
              setAnimatedStats({
                students: data.students?.length || 0,
                credentials:
                  data.credentials?.reduce(
  ...
  // Client-only effect to fetch /students directly
  useEffect(() => {
    if (!isClient) return;

    const fetchStudentsDirect = async () => {
      setStudentsDebug(null);
      ...
      const attempt = async (url: string) => {
        try {
          ...
          console.debug(`Attempted fetch to ${url}:`, res && res.status);
          return res;
        } catch (err) {
          console.debug(`Fetch to ${url} threw:`, err);
          return null;
        }
      };

      let resp = await attempt("/students");
      if (!resp || !resp.ok) {
        resp = await attempt("http://localhost:8080/students");
      }

      if (resp && resp.ok) {
        try {
          const data = await resp.json();
          console.debug("Students fetch result:", data);
          setStudentsDebug(JSON.stringify(data, null, 2));

          const arr = Array.isArray(data) ? data : [data];
          const mapped: Student[] = arr.map((s: any) => ({
            id: String(s.id ?? s.student_id ?? s.email ?? s.metamask_address),
            name:
              `${s.first_name ?? ""} ${s.last_name ?? ""}`.trim() ||
              s.email ||
              "Unknown",
            ...
            mintingStatus: "none" as const,
            joinDate: s.Account?.created_at ?? new Date().toISOString(),
            lastActivity: s.Account?.last_login_at ?? new Date().toISOString(),
          }));

          setStudents(mapped);
          setAnimatedStats({
            students: mapped.length,
            credentials: mapped.reduce((sum) => sum + 0, 0) || 0,
            pending:
              mapped.filter((s) => s.mintingStatus === "pending").length || 0,
          });
        } catch (err) {
        ...
        const msg = `Students endpoint not reachable (relative & localhost attempts)`;
        console.debug(msg);
        setStudentsDebug(msg);
      }
    };
  ...
  const handleMintSubmit = async (credentialData: any) => {
    ...
    setTimeout(() => {
      ...
      setStudents((prev) =>
        prev.map((student) =>
          student.id === selectedStudentForMint?.id
            ? { ...student, mintingStatus: "minting" as const }
            : student
        )
      );
      ...
      setTimeout(() => {
        setStudents((prev) =>
          prev.map((student) =>
            student.id === selectedStudentForMint?.id
              ? { ...student, mintingStatus: "minted" as const }
              : student
          )
        );
        setLoading(false);
      ...
      <AuthGuard>
        <div className="min-h-screen bg-black text-white flex items-center justify-center relative overflow-hidden">
          ...
          <motion.div
            ...
            style={{
              background:
                "radial-gradient(circle at 20% 80%, rgba(147, 51, 234, 0.3) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(168, 85, 247, 0.3) 0%, transparent 50%)",
            }}
          />
            ...
            <Card className="w-full max-w-md bg-gray-900/90 border-gray-800 backdrop-blur-sm">
              ...
              <CardContent>
                <p className="text-center text-gray-400 mb-4">
                  Unable to load university data. Please contact support.
                </p>
              <Button
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              >
                ...
                <Badge
                  variant="secondary"
                  className="ml-3 bg-purple-900/30 text-purple-300 border-purple-700"
                >
                  Issuer Hub
                </Badge>
              </motion.div>
          ...
          {university && (
            ...
            >
              ...
              <TabsContent value="students">
                ...
                />
                {/* Debug Info - Students Fetch Output */}
                {studentsDebug && (
                  <Card className="mt-6 bg-gray-900/90 border-gray-800 backdrop-blur-sm">
                    <CardHeader>
                      <CardTitle className="text-white flex items-center gap-2">
                        <Users className="h-5 w-5 text-purple-400" />
                        Students Fetch Debug
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
